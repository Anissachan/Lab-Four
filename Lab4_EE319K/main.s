...
; you write this
      ...
      LDR R0, =DataBuffer                        ;initialising data buffer
	  LDR R3, =EndofBuffer
	  LDR R4, [R3]
...
datainitloop
      ...
      LDR R0, =TimeBuffer                        ;initialising time buffer
...
Debug_Capture 
      ...
      LDR R0, =GPIO_PORTA_DATA_R                ;reading PA4 bit
      LDR R1, [R0]
      AND R2, R1, #0x10
        
      LDR R0, =GPIO_PORTE_DATA_R                ;reading PE0-3 bits
      LDR R1, [R0]
      AND R3, R1, #0x0F
        
      ADD R2, R2, R3                       ;combining all the bits
                                                                        
      LDR R0, =DataPt        
      LDR R1, [R0]
        
      STRB R2, [R1]                      ;dumping data into DataBuffer
        
      ADD R1, #1              ;incrementing data pointer by 1
      STR R1, [R0]
        
      LDR R0, = TimeBuffer         ;holds the address of where the buffer is located
      LDR R3, [R0]           ;R4 will hold the elapsed time
      LDR R0, = TimePt
      LDR R6, [R0]      ;R6 holds TimePt
        
      LDR R0, = NVIC_ST_CURRENT_R
      LDR R1, [R0]                   ;R1 holds the current time the timer
      LDR R0, = LastTime
      LDR R5, [R0]            ;will hold the previous time to be subtracted from
      SUBS R3, R5, R1
      AND R3, #0x00FFFFFF
      STR R3, [R6]                  ;dumping elapsed time into time buffer
      ADD R6, R6, #4          ;incrementing time pointer by 4
      MOV R5, R0          ;makes the current time the previous time
